...
import { Music, ArrowLeft, Heart, Brain, Share, ArrowRight, X, Search } from "lucide-react";
...
export default function Favorites() {
  ...
  const { favorites, isLoading, refreshData, optimisticToggleFavorite } = useDataContext();
  const isReallyLoading = isLoading && favorites.length === 0;

  // Filter favorites based on local search
  const filteredFavorites = favorites.filter(favorite =>
    favorite.songAnalysis.title.toLowerCase().includes(localSearchQuery.toLowerCase()) ||
    favorite.songAnalysis.artist.toLowerCase().includes(localSearchQuery.toLowerCase()) ||
    (favorite.songAnalysis.genre && favorite.songAnalysis.genre.toLowerCase().includes(localSearchQuery.toLowerCase()))
  );
      ...
      <div className="max-w-md mx-auto p-4">
        ...
        <div className="relative mb-6">
          ...
          <Input
            ...
            className="py-4 text-lg dark:bg-gray-800 border-gray-200 dark:border-gray-700 rounded-2xl focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-[#27417d] ml-[0px] mr-[0px] mt-[2px] mb-[2px] pl-[40.25px] pr-[40.25px] pt-[18px] pb-[18px]"
          />
        </div>
        ...
        {filteredFavorites.length > 0 ? (
          <>
            <div className="mb-4">
              <h2 className="text-lg font-semibold text-white">
                ...
                {localSearchQuery && ` (filtered from ${favorites.length})`}
              </h2>
            </div>
            <div className="space-y-3">
              {filteredFavorites.map((favorite: Favorite & { songAnalysis: SongAnalysis }) => (
                <Card key={favorite.id} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors rounded-md" onClick={() => setSelectedAnalysis(favorite.songAnalysis)}>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium dark:text-white text-[#ffffff]">
                          {favorite.songAnalysis.title}
                        </h4>
                        <p className="dark:text-gray-400 font-medium text-[15px] text-[#beccde]">
                          {favorite.songAnalysis.artist}
                        </p>
                        {favorite.songAnalysis.genre && (
                          <p className="text-xs text-[#beccde]/80 mt-1">
                            {favorite.songAnalysis.genre} • {favorite.songAnalysis.yearReleased}
                          </p>
                        )}
                      </div>
                      <div className="flex items-center space-x-2">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleFavorite(favorite.songAnalysis.id);
                          }}
                          disabled={removeFavoriteMutation.isPending || favoriteMutation.isPending}
                        >
                          <Heart className="w-4 h-4 fill-red-500 text-red-500" />
                        </Button>
                        <span className="text-xs text-[#beccde]">
                          {favorite.createdAt 
                            ? format(new Date(favorite.createdAt), "dd MMM")
                            : ""}
                        </span>
          ...
          <EmptyState
            ...
            description={`No favorites match "${localSearchQuery}"`}
          />
        ) : (
          <EmptyState
            icon={<Heart className="w-12 h-12 text-purple-400" />}
            title="No Favorites Yet"
            description="Start adding songs to your favorites by tapping the heart icon"
            action={
